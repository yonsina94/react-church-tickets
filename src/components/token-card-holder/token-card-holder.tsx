...
export class TokenCardHolder extends Component<{tokenMainCode:string}>{
    ...
    loadData = async (tokenMainCode: string) => {
        ...
        if(resp.status >= 200 && resp.status < 300 /*resp*/){
            const amount = resp.data.amount; //resp.data.amount;
                        ...
                        <div>
                      {
                           data.codes.map(v=>(
                            <div className="holder-container">
                                ...
                                tokenDescription={/*resp.data.tokenDescription*/ data.details.tokenDescription}
                                tokenCode={`${/*resp.data.tokenMainCode*/ data.details.tokenMainCode}-${v}`}
                                />
                            </div>
                        ))
                      }
